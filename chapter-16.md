...
定义规则。这是谜题设计的关键部分。大多数谜题都有四个方面的特征：棋盘（是网格？ 是网络？ 是不规则的？还是根本就没有棋盘？）、棋子（形状如何？上面有什么图画？它们还有什么其他属性？它们来自哪里？）、走棋（允许走什么，不允许走什么？它们是按顺序进行还是同时进行？它们有什么副作用？），以及目标或胜利条件（是否必须完全匹配，还是部分匹配也可以？）
